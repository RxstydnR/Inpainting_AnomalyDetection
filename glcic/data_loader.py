...
class TrainDataLoader(tf.keras.utils.Sequence):
    ...
    def __init__(self, dir_path, batch_size=32, phase="phase1",
                 img_shape=(256,256), box_size=(128,128),
                 gray=False, augmentation=False, transform=None, shuffle=True):

        self.img_files = glob.glob(os.path.join(dir_path,'*.*'))
    ...
    def __getitem__(self, index):
        ...
        if self.phase=="phase1":
            return [X_masked, X_mask],[X_img]
        elif self.phase=="phase2" or self.phase=="phase3":
            return [X_masked,X_mask,X_mask_area,X_img] # (x1,y1,x2,y2)        
        else:
...
class TestDataLoader(tf.keras.utils.Sequence):
    ...
    def __init__(self, dir_path, img_shape=(256,256), box_size=(128,128), batch_size=1, gray=False):
